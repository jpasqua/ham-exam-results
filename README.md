...
This tool can grade old exams; that is, exams from old question pools. It will automatically determine the right question pool based on the date the exam was started.
...
- Sine the question pools are updated periodically, the names of these files include the last day on which the exam pool is valid.
- The names include the exam class and last eligible date in this format: `<class>_YYYY-MM-DD.json` (for example, `technician_2026-06-30.json`). At runtime, the app selects the oldest pool whose cutoff date is on or after the exam date.
